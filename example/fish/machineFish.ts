 ...
 */
import { Tag, Fish, FishId, Pond } from '@actyx/pond'
...
const emitMachineState = (pond: Pond, machineId: string, state: IdleStates['stateType']) =>
  pond.emit(tags.machine.withId(machineId).and(tags.state.withId(machineId)), {
    eventType: 'setState',
    machine: machineId,
    state,
  })
